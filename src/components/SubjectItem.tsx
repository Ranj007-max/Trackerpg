...
const LectureStatusChanger: React.FC<{ lecture: Lecture; onChange: (newStatus: LectureStatus) => void; }> = ({ lecture, onChange }) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute z-10 bottom-full mb-2 w-32 bg-secondary-dark border border-border-color rounded-md shadow-lg">
          {Object.values(LectureStatus).map(status => (
            <button key={status} onClick={() => { onChange(status); setIsOpen(false); }} className="block w-full text-left px-3 py-1.5 text-sm text-text-primary-dark hover:bg-primary-dark">
              {status}
            </button>
          ))}
        </div>
...
const SubjectItem: React.FC<SubjectItemProps> = ({ subject, onUpdate, onDelete }) => {
  ...
  return (
    <div className="bg-secondary-light dark:bg-secondary-dark rounded-lg shadow-lg overflow-hidden border border-border-color">
      <div className="p-4 flex items-center justify-between cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex-1">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-xl font-bold">{subject.name}</h3>
            <span className="text-sm font-semibold text-accent-light dark:text-accent-dark">{stats.percentage}% <span className="text-text-secondary-light dark:text-text-secondary-dark font-normal">({stats.completed}/{stats.target})</span></span>
          </div>
          <ProgressiveBar stats={stats} total={stats.target} />
        </div>
        <div className="flex items-center ml-4">
          <button onClick={(e) => { e.stopPropagation(); setEditSubjectModalOpen(true); }} className="p-2 text-text-secondary-light dark:text-text-secondary-dark hover:text-text-primary-light dark:hover:text-text-primary-dark hover:bg-secondary-light dark:hover:bg-primary-dark rounded-full transition-colors" title="Edit Subject"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg></button>
          <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteOpen(true); }} className="p-2 text-text-secondary-light dark:text-text-secondary-dark hover:text-red-500 hover:bg-secondary-light dark:hover:bg-primary-dark rounded-full transition-colors" title="Delete Subject"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 text-text-secondary-light dark:text-text-secondary-dark transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="p-4 border-t border-border-color">
          <div className="flex justify-end mb-4">
              <button onClick={() => setAddChapterModalOpen(true)} className="bg-primary-light dark:bg-primary-dark hover:bg-secondary-light dark:hover:bg-secondary-dark text-sm font-semibold py-2 px-3 rounded-lg shadow-sm transition-colors border border-border-color">
                + Add Chapter
              </button>
          </div>
          {subject.chapters.length > 0 ? (
            <div className="space-y-3">
              {subject.chapters.map(chapter => (
                <div key={chapter.id} className="bg-primary-light dark:bg-primary-dark p-3 rounded-md border border-border-color">
                    <div className="flex justify-between items-center mb-2">
                        <h4 className="font-semibold">{chapter.name}</h4>
                        <div>
                            <button onClick={() => openLectureModal(chapter.id)} className="px-2 py-1 text-xs bg-accent-light dark:bg-accent-dark hover:opacity-90 text-white rounded-md mr-2">+ Add Lecture</button>
                            <button onClick={() => handleDeleteChapter(chapter.id)} className="px-2 py-1 text-xs bg-red-600 hover:bg-red-700 text-white rounded-md">Delete Chapter</button>
                        </div>
                    </div>
                    <div>
                        {chapter.lectures.length > 0 ? (
                            <div className="space-y-2">
                               {chapter.lectures.map(lecture => (
                                   <div key={lecture.id} className="text-sm p-3 bg-secondary-light dark:bg-secondary-dark rounded-md border border-border-color">
                                       <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                                           <div className="flex items-center gap-2 flex-grow">
                                                <button onClick={() => handleToggleBookmark(chapter.id, lecture.id)} className="p-1 text-text-secondary-light dark:text-text-secondary-dark hover:text-yellow-400" title="Toggle Bookmark">
                                           ...
                                           <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-text-secondary-light dark:text-text-secondary-dark text-xs">
                                               {lecture.platform && (
                                           ...
                                           <div className="flex items-center justify-between md:justify-end gap-3 shrink-0">
                                               ...
                                               <div className="flex items-center">
                                                   {lecture.videoUrl && (
                                                      <a href={lecture.videoUrl} target="_blank" rel="noopener noreferrer" className="p-1 text-text-secondary-light dark:text-text-secondary-dark hover:text-accent-light dark:hover:text-accent-dark" title="Open Video Link">
                                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" /></svg>
                                                      </a>
                                                    )}
                                                   <button onClick={() => openLectureModal(chapter.id, lecture)} className="p-1 text-text-secondary-light dark:text-text-secondary-dark hover:text-sky-400" title="Edit Lecture"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                                                   <button onClick={() => handleDeleteLecture(chapter.id, lecture.id)} className="p-1 text-text-secondary-light dark:text-text-secondary-dark hover:text-red-400" title="Delete Lecture"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                               </div>
                                           </div>
                            ...
                            <p className="text-text-secondary-light dark:text-text-secondary-dark text-sm italic">No lectures added for this chapter.</p>
                        )}
          ...
          ) : <p className="text-text-secondary-light dark:text-text-secondary-dark text-center">No chapters added yet.</p>}
        </div>
      )}
