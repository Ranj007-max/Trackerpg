...
const statusConfig: { [key in LectureStatus]: { color: string; label: string } } = {
  [LectureStatus.NotStarted]: { color: 'bg-gray-500/20 text-gray-300', label: 'Not Started' },
  ...
  [LectureStatus.Revision]: { color: 'bg-purple-500/20 text-purple-300', label: 'Revision' },
};
...
const SubjectItem: React.FC<SubjectItemProps> = ({ subject, onUpdate, onDelete }) => {
  ...
  return (
    <div className="bg-background-secondary rounded-2xl shadow-sm overflow-hidden border border-border-color">
      <div className="p-4 flex items-center justify-between cursor-pointer group" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex-1">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-xl font-bold text-text-primary">{subject.name}</h3>
            <span className="text-sm font-semibold text-accent">{stats.percentage}% <span className="text-text-secondary font-normal">({stats.completed}/{stats.target})</span></span>
          </div>
          <ProgressiveBar stats={stats} total={stats.target} />
        </div>
        <div className="flex items-center ml-4">
          <button onClick={(e) => { e.stopPropagation(); setEditSubjectModalOpen(true); }} className="p-2 text-text-secondary hover:text-text-primary hover:bg-background-primary rounded-full transition-colors" title="Edit Subject"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg></button>
          <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteOpen(true); }} className="p-2 text-text-secondary hover:text-red-500 hover:bg-background-primary rounded-full transition-colors" title="Delete Subject"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 text-text-secondary transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="p-4 border-t border-border-color">
          <div className="flex justify-end mb-4">
              <button onClick={() => setAddChapterModalOpen(true)} className="bg-background-primary hover:bg-background-secondary text-sm font-semibold py-2 px-3 rounded-lg shadow-sm transition-colors border border-border-color">
                + Add Chapter
              </button>
          </div>
          {subject.chapters.length > 0 ? (
            <div className="space-y-3">
              {subject.chapters.map(chapter => (
                <div key={chapter.id} className="bg-background-primary p-3 rounded-lg border border-border-color">
                    <div className="flex justify-between items-center mb-2">
                        <h4 className="font-semibold text-text-primary">{chapter.name}</h4>
                        <div>
                            <button onClick={() => openLectureModal(chapter.id)} className="px-2 py-1 text-xs bg-accent hover:opacity-90 text-white rounded-md mr-2">+ Add Lecture</button>
                        ...
                        {chapter.lectures.length > 0 ? (
                            <div className="space-y-2">
                               {chapter.lectures.map(lecture => (
                                   <div key={lecture.id} className="text-sm p-3 bg-background-secondary rounded-lg border border-border-color">
                                       <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                                           <div className="flex items-center gap-2 flex-grow">
                                                <button onClick={() => handleToggleBookmark(chapter.id, lecture.id)} className="p-1 text-text-secondary hover:text-yellow-400" title="Toggle Bookmark">
                                                ...
                                                <div className="font-medium text-text-primary">{lecture.name}</div>
                                           </div>
                                           <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-text-secondary text-xs">
                                               {lecture.platform && (
                            ...
                            <p className="text-text-secondary text-sm italic">No lectures added for this chapter.</p>
                        )}
          ...
          ) : <p className="text-text-secondary text-center">No chapters added yet.</p>}
        </div>
      )}
