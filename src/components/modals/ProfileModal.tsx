    ...
    <Modal title={isLocked ? "Your Profile" : "Edit Your Profile"} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="absolute top-4 right-4 flex items-center">
            <span className="text-sm text-text-secondary mr-2">{isLocked ? 'Locked' : 'Unlocked'}</span>
            <button type="button" onClick={() => setIsLocked(!isLocked)} className="p-2 rounded-full hover:bg-background-secondary transition-colors">
                {isLocked ? (
        ...
        <div>
          <label htmlFor="name" className="block text-sm font-medium text-text-secondary mb-1">Student Name</label>
          <input
            id="name"
            name="name"
            type="text"
            ...
            className="w-full bg-background-primary border border-border-color rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-accent"
        ...
        <div>
          <label htmlFor="targetYear" className="block text-sm font-medium text-text-secondary mb-1">NEET PG Target Year</label>
          <input
            id="targetYear"
            name="targetYear"
            type="text"
        ...
        <div>
          <label htmlFor="mbbsYear" className="block text-sm font-medium text-text-secondary mb-1">Current MBBS Year</label>
          <select
            id="mbbsYear"
            name="mbbsYear"
        ...
        <div className="flex justify-end space-x-3 pt-2">
          ...
          {!isLocked && <button type="submit" className="px-4 py-2 rounded-md bg-accent hover:opacity-90 text-white transition-colors font-semibold">Save Profile</button>}
        </div>
      </form>
    </Modal>
  );
